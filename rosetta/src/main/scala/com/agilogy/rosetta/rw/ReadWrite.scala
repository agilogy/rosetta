...
trait ReadWrite[NR[_], NW[_], E, A] extends Read[NR, E, A] with Write[NW, A] {
  def imap[B](f: A => B)(g: B => A): ReadWrite[NR, NW, E, B] =
    ReadWrite(this.map(f), this.contramap(g))
  def iAndThen[B](f: A => Either[E, B])(g: B => A): ReadWrite[NR, NW, E, B] =
    ReadWrite(this.andThen(f), this.contramap(g))
}

object ReadWrite {
  def apply[NR[_], NW[_], E, A](reader: Read[NR, E, A], writer: Write[NW, A]): ReadWrite[NR, NW, E, A] =
